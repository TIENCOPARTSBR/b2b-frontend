...
const Purchase = ({ order, orderResume }: PurchaseInterface) => {
    ...
    return (
        <div className="w-fill mt-50px flex flex-wrap md:justify-between items-center">
            <h2 className="w-full mb-5 text-20px font-semibold font-inter text-black">Order Resume</h2>

            <div className="w-full flex items-center p-35px rounded-8px border-1 border-grey_six items-end flex-wrap">
                <div className="w-full md:w-1/4">
                    <h2 className="w-full mb-4 text-16px font-bold font-inter text-black">Total Cost</h2>
                    <p className="w-full mb-4 text-18px font-regular font-inter text-black">USD $ {orderResume?.total_price ?? '---'}</p>
                </div>
                ...
                <div className="w-full md:w-1/4 flex justify-end">
                    <button onClick={handleRequestPurchase} className="w-150px h-40px py-1 bg-green_one flex items-center justify-center rounded-60px">

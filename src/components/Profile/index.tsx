...
const Profile = () => {
    ...
    return (
        <div className="relative" ref={profileRef}>
            <button
                className={`w-45px h-45px rounded-3xl flex items-center justify-center hover:bg-yellow_one transition-all ease-in-out duration-300 ${
                    isOpen ? "bg-yellow_one" : ""
                }`}
                onClick={() => {
                    handleClick();
                }}
            >
                <Image src="/icon/icon-profile.svg" alt="Icon profile" width="24" height="24" />
            </button>
            ...
            {isOpen && (
                <div className="w-350px bg-white absolute top-50px right-10px p-25px flex flex-col rounded-lg border-1 border-grey_one shadow-profile">
                    ...
                    <Link href={`/change-password`} className="w-100 flex items-center justify-between py-3 pb-6">
                        ...
                        <Image src="/icon/icon-arrow-right.svg" alt="Icon arrow right" width="16" height="16" className="mr-2 mt-0.5" />
                    </Link>
